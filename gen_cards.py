...
base_cards = [
("Read Receipts","friendship","Your friend texts you asking if you are okay. You opened it three hours ago. You were not busy. You were emotionally buffering.","Reply sorry just saw this even though the receipt is sitting there like a courtroom witness.","Ignore it longer and hope time turns into communication.","Reply honestly: I did not know what to say, but I am not doing great.","Send a meme and hope comedy counts as communication.","Time did not fix it. Time just sat there and watched."),
...
def scores(category, kind):
    d = {"temptation":0,"gossip":0,"pride":0,"fear":0,"relationships":0,"chaos":0,"growth":0}
    key = stat_map.get(category, "chaos")
    if kind == "chaos":
        d[key] = min(d.get(key,0)+5, 10); d["chaos"] += 4; d["pride"] += 2; d["relationships"] += 2; return d, 8
    if kind == "avoid":
        d[key] = min(d.get(key,0)+2, 10); d["fear"] += 3; d["chaos"] += 1; return d, 4
    if kind == "healthy":
        d["growth"] += 5; return d, 0
    d[key] = min(d.get(key,0)+1, 10); d["fear"] += 1; d["growth"] += 3; d["chaos"] += 1; return d, 2
...
for row in base_cards:
    title, cat, scenario, chaos, avoid, healthy, middle, roast = row
    for suffix, age, add in variants:
        if len(cards) >= 250:
            break
        full_title = title if not suffix else title + ": " + suffix
        opts = [("chaos", chaos), ("avoid", avoid), ("healthy", healthy), ("middle", middle)]
        random.shuffle(opts)
        options = []
        for label, (kind, text) in zip(["A","B","C","D"], opts):
            sc, mis = scores(cat, kind)
            if suffix == "After Midnight" and kind == "chaos":
                sc["temptation"] = min(sc.get("temptation",0)+1, 10)
                mis = min(10, mis + 1)
            options.append({
                "label": label,
                "text": text,
                "scores": sc,
                "mischief": mis,
                "breakdown": random.choice(breakdowns[kind]),
                "roast": roast,
                "reflection": random.choice(reflections[kind]),
                "verdict": random.choice(verdicts[kind])
            })
        cards.append({
            "id": slug(full_title) + "-" + str(idx).zfill(3),
            "category": cat,
            "ageRating": age,
            "title": full_title,
...
js = "// Full Mischievia card pack — " + str(len(cards)) + " cards\n\nconst mischieviaCards = "
...
out = Path("c:/Users/alexs/Downloads/mischievia-vsc-project/mischievia-vsc-project/cards.js")
